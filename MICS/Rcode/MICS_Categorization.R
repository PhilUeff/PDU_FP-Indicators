...
MaritalStatusCategories<-function(df,choice){
  if(choice == "Both"){
    df<-MaritalStatusCategories(df,"General")
    ...
    formerly <- c(30:35)
    never <- c(10)
    ##Categorize for Formerly/Never Married Women
    df["MSTATNF"]<-NA
...
MethodCategories <- function(df){
  ...
  ###Specific methods are separated into different variables
    ...
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSIMP == 1)] <- 102
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSMST == 1)] <- 106
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSIUS == 1)] <- 101
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSFST == 1)] <- 105
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSIUD == 1)] <- 101 # Some IUS might have been classified as IUD in MICS as differentiation not made. Trussel: IUS > FST > IUD 
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSINJ == 1)] <- 120 # Not in Trussel
    ...
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSPILL == 1)] <- 110
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSPAT == 1)] <- 107
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSRING == 1)] <- 108
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSCONM == 1)] <- 103
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSRHY == 1)] <- 210
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSDIA == 1)] <- 130
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSCONF == 1)] <- 104
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSLAM == 1)] <- 140 # Not in Trussel
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSWD == 1)] <- 220
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSFOA == 1)] <- 135 # Spermicides in Trussel
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSEC == 1)] <- 150 # Not in Trussel
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSBF == 1)] <- 141 # Not in Trussel
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSOTH == 1)] <- 300
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSNSP == 1)] <- 301
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSOTHMOD == 1)] <- 302
    df$FPMETHOD[which(is.na(df$FPMETHOD) & df$FPNOWUSOTHTRAD == 1)] <- 303
  ...
  traditional <- c(141, 200, 210:217, 220, 230:236, 303)
  ...
  df$FPTYPE_LAB <- mapvalues(df$FPTYPE, from = c(1, 2, 3), to = c("Using_modern", "Using_traditional", "Not_using"), warn_missing = F)
